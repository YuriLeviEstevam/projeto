...
if ($method === 'POST' && $path === '/patients') {
    ...
    if ($cpf !== '' && !validarCPF($cpf)) $err[] = 'CPF inválido.';
    if ($phone !== '' && !preg_match('/^\d{10,11}$/', preg_replace('/\D/', '', $phone))) $err[] = 'Telefone inválido.';
    if ($cell !== '' && !preg_match('/^\d{10,11}$/', preg_replace('/\D/', '', $cell))) $err[] = 'Celular inválido.';
    if ($birth !== '') {
    ...
    if ($err) {
        $msg = '<div class="alert error"><strong>Erro:</strong><ul><li>' .
               ...
               '</li></ul></div>';
        echo page_form($msg, compact('name','birth','phone','cell','email','cpf'));
        exit;
    }
    ...
    try {
        ...
        echo page_form('<div class="alert error"><strong>Erro ao salvar:</strong> ' . h($e->getMessage()) . '</div>', compact('name','birth','phone','cell','email','cpf'));
        exit;
    }
}
...
input[type=text],input[type=date],input[type=email],input[type=tel]{width:100%;padding:10px;border:1px solid #ddd;border-radius:8px}
button{padding:12px 16px;border:0;border-radius:8px;cursor:pointer}
button.primary{background:#0d6efd;color:#fff}
.alert{padding:12px 14px;border-radius:8px;margin:8px 0 4px}
.alert.success{background:#e6f4ea;color:#1e7e34;border:1px solid #b7e1c1}
.alert.error{background:#fdecea;color:#a11;border:1px solid #f5c2c7}
